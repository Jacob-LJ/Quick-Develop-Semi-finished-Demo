...
- (void)checkoutBackgroundView {
    ...
    if (![self kiz_backgroundView]) {
        ...
        UIView *bgView = [[UIView alloc] initWithFrame:CGRectMake(0, -20, HLMScreenW, self.bounds.size.height + 20)];
        bgView.backgroundColor = [UIColor whiteColor];
        bgView.userInteractionEnabled = NO;
        [self insertSubview:bgView atIndex:0];
        [self kiz_setBackgroundView:bgView];
        
        UIView *seperateView = [[UIView alloc] initWithFrame:CGRectMake(0, bgView.hlm_height, HLMScreenW, 0.5)];
        [bgView addSubview:seperateView];
        seperateView.backgroundColor = RGBA(204, 204, 204, 1);
    }
